...
spe_pseudo <- spe_pseudo[, spe_pseudo$age_bin %in% c("Teen")]

# Load modeling results
modeling_results <- readRDS(file = here::here("processed-data", "pseudobulk_spe", "teen_modeling_results.rds"))
...
cluster_labels <- as.vector(c(
    rep("Cluster_1", 2), rep("Cluster_2", 2), rep("GCL", 2), rep("SGZ", 2),
...
pdf(file = here::here("plots", "pseudobulked", "teen_enrichment_heatmap.pdf"), width = 8, height = 8)
pheatmap(
    exprs_heatmap,
    cluster_rows = TRUE,
    cluster_cols = TRUE,
    ...
    fontsize_row = 9,
    main = "logcounts from enrichment model of Teen top 10 genes/cluster",
)
dev.off()
