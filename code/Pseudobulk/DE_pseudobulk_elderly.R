...
elderly_spe_pseudo <- spe_pseudo[, spe_pseudo$age_bin %in% c("Elderly")]

# Format spe object for DE models
colData(elderly_spe_pseudo) <- colData(elderly_spe_pseudo)[, sort(c(
    "age",
    "sample_id",
    "BayesSpace",
    "sex",
...
colData(elderly_spe_pseudo)

colData(elderly_spe_pseudo)$ncells <- as.numeric(colData(elderly_spe_pseudo)$ncells)
colData(elderly_spe_pseudo)$race <- as.factor(colData(elderly_spe_pseudo)$race)
colData(elderly_spe_pseudo)$sample_id <- as.factor(colData(elderly_spe_pseudo)$sample_id)
colData(elderly_spe_pseudo)$sex <- as.factor(colData(elderly_spe_pseudo)$sex)

colData(elderly_spe_pseudo)

# Drop things we don't need
spatialCoords(elderly_spe_pseudo) <- NULL
imgData(elderly_spe_pseudo) <- NULL

# Extract the data
mat <- assays(elderly_spe_pseudo)$logcounts

# Matrix for a regression-like model
mod <- with(colData(elderly_spe_pseudo), model.matrix(~ 0 + BayesSpace))

# Compute correlation
corfit <- duplicateCorrelation(mat, mod, block = elderly_spe_pseudo$sample_id)

######### ENRICHMENT t-stats ######################

cluster_id <- splitit(elderly_spe_pseudo$BayesSpace)

eb0_list <- lapply(cluster_id, function(x) {
    res <- rep(0, ncol(elderly_spe_pseudo))
    res[x] <- 1
    m <- model.matrix(~res)
    eBayes(
        lmFit(
            mat,
            design = m,
            block = elderly_spe_pseudo$sample_id,
...
ebF_list <-
    lapply(list("full" = elderly_spe_pseudo), fit_f_model)
...
f_stats$ensembl <- rownames(elderly_spe_pseudo)
f_stats$gene <- rowData(elderly_spe_pseudo)$gene_name
rownames(f_stats) <- NULL

head(f_stats)

save(f_stats, eb0_list, eb_contrasts,
    file = here::here("processed-data", "pseudobulk_spe", "elderly_bayesspace_cluster_modeling_results.Rdata")
)
