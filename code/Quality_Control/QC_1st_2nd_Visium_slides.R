...
stopifnot(nrow(spe$key) == length(slide_df))

colData(spe)$slide <-
    factor(slide_df$slide,
        levels = c("V10B01088", "V11L05334", "V11D01387"))
...
vis_grid_clus(
    ...
    pdf = here::here(
        "plots",
        "QC_plots",
        "First&Second_slides_in_tissue_grid.pdf"
    ),
    sort_clust = FALSE,
...
is_mito <-
    grepl("(^MT-)|(^mt-)", rowData(first_second_spe)$gene_name)
table(is_mito)
rowData(first_second_spe)$gene_name[is_mito]

# calculate per-spot QC metrics and store in colData
first_second_spe <-
    addPerCellQC(first_second_spe, subsets = list(mito = is_mito))
head(colData(first_second_spe))

# Create QC Metrics
qcstats <- perCellQCMetrics(first_second_spe, subsets = list(Mito = which(seqnames(first_second_spe) == "chrM")))
...
qcfilter <- DataFrame(
    low_lib_size = isOutlier(
        ...
        type = "lower",
        log = TRUE,
        batch = first_second_spe$sample_id
    ),
...
pdf(
    file = here::here(
        "plots",
        "QC_plots",
        "First&Second_slides_histograms_allSpots.pdf"
    ),
    width = 10.5,
    height = 2.5
)
par(mfrow = c(1, 5))
hist(colData(first_second_spe)$sum_umi,
    xlab = "sum",
    main = "UMIs per spot all donors")
hist(colData(first_second_spe)$sum_gene,
    xlab = "detected",
    main = "Genes per spot all donors")
hist(colData(first_second_spe)$subsets_mito_percent,
    xlab = "percent mito",
    main = "Percent mito all donors")
hist(colData(first_second_spe)$NBW,
    xlab = "no. cells",
    main = "No. cells per spot all donors")
hist(colData(first_second_spe)$PBW,
    xlab = "% cells",
    main = "% cells per spot all donors")
par(mfrow = c(1, 1))
...
pdf(
    here(
        "plots",
        "QC_plots",
        "First&Second_slides_QC_Violinplots_capture_area.pdf"
    ),
    width = 21
)
## Mito rate
plotColData(first_second_spe,
    x = "sample_id",
...
for (i in colnames(qcfilter)) {
    vis_grid_clus(
        ...
        pdf = here::here(
            "plots",
            "QC_plots",
            paste0("First&Second_slides_scran_", i, ".pdf")
        ),
        sort_clust = FALSE,
...
first_second_spe <-
    first_second_spe[, colData(first_second_spe)$scran_discard == FALSE]
dim(first_second_spe)

# calculate logcounts (log-transformed normalized counts) and store in object
first_second_spe$scran_quick_cluster <-
    quickCluster(first_second_spe, block = first_second_spe$sample_id)
first_second_spe <-
    computeSumFactors(first_second_spe, clusters = first_second_spe$scran_quick_cluster)
