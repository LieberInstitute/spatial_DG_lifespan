...
# Sub-clustering of sce object for Endo_Mural
...
# Subset for Endo_Mural cells

sce_full <- sce

# select Endo_Mural clusters
# (identified based on marker expression heatmap from previous script)
clus_select <- c("Endo_Mural")
...
# secondary clustering of Endo_Mural cells
...
clus <- clusterCells(
  ...
  BLUSPARAM = TwoStepParam(
    ...
    second = NNGraphParam(k = 25, cluster.fun = "leiden")
  )
)
...
pdf(file = here::here("plots", "sce_plots", "Endo_Mural_cluster_markers_sce.pdf"))

plotExpression(sce, features=c("CLDN5", "FLT1", "RBPMS", "PAX6", "HOPX", "EOMES"),
    x="label", colour_by="label")

dev.off()

# Plot UMAP after clustering
pdf(file = here::here("plots", "sce_plots", "DG_UMAP_Endo_Mural_clusters_sce.pdf"))

plotReducedDim(sce, dimred = "UMAP.HARMONY", colour_by = "label",
    point_alpha = 0.3, point_size = 0.5) +
  ggtitle("Unsupervised clustering of Endo_Mural clusters")

dev.off()

# Create heatmap for markers of mean expresion with z-scores
markers <- c("CLDN5", "FLT1", "RBPMS", "PAX6", "HOPX", "EOMES")
...
pdf(file = here::here("plots", "sce_plots", "Heatmap_Endo_Mural_markers_sce.pdf"), width = 12, height = 8)

Heatmap(
  hm_mat,
  name = "z-score",
  column_title = "Endo_Mural markers",
...
clus_Endo_Mural <- rep(NA, ncol(sce_full))
names(clus_Endo_Mural) <- colnames(sce_full)
clus_Endo_Mural[colnames(sce)] <- colData(sce)$label

colData(sce_full)$label_Endo_Mural <- clus_Endo_Mural

# check
table(colData(sce_full)$label)
table(colData(sce_full)$label_Endo_Mural)
table(colData(sce_full)$label_Endo_Mural, useNA = "always")
