...
# Stacked bar plot of cell-type proportions across all samples

# order spe observations according to age
spe <- spe[, order(spe$age)]

prop_df <- as.data.frame(colData(spe)[, 44:68])
prop_df$sample_id <- spe$sample_id
prop_df$age <- spe$age

df_proportions <- prop_df %>%
  group_by(sample_id) %>%
  summarize(across(starts_with("meanscell_abundance_w_sf_"), ~sum(.) / sum(c_across(starts_with("meanscell_abundance_w_sf_")))))

# Reshape the data for plotting
df_proportions_long <- df_proportions %>%
  pivot_longer(
    cols = starts_with("meanscell_abundance_w_sf_"),
    names_to = "Variable",
    values_to = "Proportion"
  )

df_proportions_long <- df_proportions_long %>%
  mutate(Variable = str_replace(Variable, "meanscell_abundance_w_sf_", ""))

sample_prop <- as.data.frame(df_proportions_long)
sample_prop$age <- c(rep(15.16, 25), rep(17.94, 25), rep(48.20, 25), rep(47.50, 25),
    rep(73.90, 25), rep(92.25, 25), rep(76.38, 25), rep(1.84, 25), rep(18.41, 25),
    rep(33.40, 25), rep(17.42, 25), rep(0.31, 25), rep(0.58, 25), rep(37.30, 25),
    rep(1.05, 25), rep(0.18, 25))

sample_prop <- sample_prop %>%
  arrange(age)

# Ordering by age did not correctly plot in age order so factoring will help
...
pdf(file = here::here("plots", "Cell_Type_Deconvolution", "cell2loc_proportions_barplot.pdf"), width = 9, height = 6)

ggplot(sample_prop, aes(x = factor(sample_id, level = level_order), y = Proportion, fill = Variable)) +
    geom_bar(stat = "identity") +
    scale_fill_manual(values = cell_colors) +
    labs(title = "Proportions of cell type by sample", y = "Proportion", x = "sample_id",
        fill = "Cell type") +
    theme(text = element_text(size = 14), axis.text = element_text(size = 14),
        axis.text.x = element_text(angle = 90), panel.background = element_blank())
