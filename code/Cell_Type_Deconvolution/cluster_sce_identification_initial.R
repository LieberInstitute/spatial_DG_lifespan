...
pdf(file = here::here("plots", "sce_plots", "Initial heatmap_markers_sce.pdf"), width = 12, height = 8)

Heatmap(
  hm_mat,
  name = "z-score",
  column_title = "Initial DG clusters",
...
label_merged <- fct_collapse(colData(sce)$label,
  ...
  Glia = as.character(cluster_pops[[3]]),
  Oligo = as.character(cluster_pops[[4]]),
  Immune = as.character(cluster_pops[[5]]),
  OPCs = as.character(cluster_pops[[6]]),
  Mixed = as.character(cluster_pops[[7]])
    )

label_merged <- fct_relevel(label_merged,
  c("ExctN", "InhbN", "Glia", "Oligo", "Immune",
      "OPCs", "Mixed"))

table(label_merged)
#label_merged
# ExctN  InhbN   Glia  Oligo Immune   OPCs  Mixed
# 60596   5697  27202  70598  13534  12802  18846
...
pdf(file = here::here("plots", "sce_plots", "Initial_merged_cluster_plot_sce.pdf"))
...
plotReducedDim(sce, dimred = "UMAP.HARMONY", colour_by = "label_merged") +
  ...
  theme_classic() +
  ggtitle("Combined HPC snRNAseq datasets initial merged clustering")
